<table id="search-table4">
    ...
    <tbody>
        <?php
            $quality_query = "SELECT * FROM types WHERE `section` = 'quality' ORDER BY `id` DESC;";
            $qualityrun = $conn->query($quality_query);
            if($qualityrun->num_rows > 0){
                while($quality = $qualityrun->fetch_assoc()){
                    ?>
                    <tr>
                        <td><?php echo $quality['id']; ?></td>
                        <td><?php echo $quality['name']; ?></td>
                        <td class="italic"><?php echo $quality['slug']; ?></td>
                        ...
                        <td><?php echo date("j-M-Y g:i:sA", strtotime($quality['created_at'])); ?></td>
                        <td class="flex items-center justify-start gap-1">
                            <button onclick="openEditModal<?php echo $quality['id']; ?>()" class="transition-all duration-100 ease-in-out p-2 rounded-xl bg-blue-500 hover:bg-blue-700"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-square-pen-icon lucide-square-pen"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.375 2.625a1 1 0 0 1 3 3l-9.013 9.014a2 2 0 0 1-.853.505l-2.873.84a.5.5 0 0 1-.62-.62l.84-2.873a2 2 0 0 1 .506-.852z"/></svg></button>

                            <a class="transition-all duration-100 ease-in-out p-2 rounded-xl bg-red-500 hover:bg-red-700" href="./quality/quality_delete.php?slug=<?php echo $quality['slug']; ?>" onclick="return confirm('Are you sure you want to delete this quality?');"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash2-icon lucide-trash-2"><path d="M10 11v6"/><path d="M14 11v6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></a>
                        </td>
                    </tr>

                    <div id="editModal<?php echo $quality['id']; ?>" class="fixed inset-0 w-full flex items-center justify-center bg-black/80 opacity-0 pointer-events-none transition-opacity duration-300">
                        <div id="editmodalBox<?php echo $quality['id']; ?>" class="bg-gray-800 p-6 rounded-xl shadow-lg transform scale-95 transition-all duration-100 w-2/6">
                            <h2 class="text-2xl font-semibold pb-2 mb-2 border-b border-gray-500">Edit Quality</h2>
                            
                            <form action="./quality/quality_update.php" method="POST" class="w-full grid grid-cols-12 gap-4">
                                <input class="w-full rounded-xl text-black" type="text" name="dataid" id="dataid" value="<?php echo $quality['id']; ?>" required readonly hidden>
                                ...
                                <div class="col-span-12 grid grid-cols-12 gap-2 text-left mb-2">
                                    <div class="col-span-8">
                                        ...
                                        <input class="w-full rounded-xl text-black" type="text" name="name" id="name" placeholder="Type quality name" value="<?php echo $quality['name']; ?>" required>
                                    </div>
                                    ...
                                    <div class="col-span-4">
                                        ...
                                        <select class="w-full rounded-xl text-black" name="status" id="status" required>
                                            <option class="text-black" value="<?php echo $quality['status'];?>" selected hidden><?php echo ($quality['status'] == 1) ? "Active" : "Inactive"; ?></option>
                                ...
                                <div class="col-span-12 text-left mb-2 flex flex-col items-center">
                                    ...
                                    <?php
                                        if($quality['banner'] == NULL){
                                            ?><p class="text-sm text-gray-200 italic">No banner found</p><?php
                                        }
                                        else{
                                            ?><img class="w-auto h-48 rounded-xl" src="<?php echo $quality['banner']; ?>" alt=""><?php
                                        }
                                    ?>
                                    <input class="w-full mt-1 w-full rounded-xl text-black" type="text" name="banner" id="banner" placeholder="Enter quality banner URL" value="<?php echo $quality['banner']; ?>">
                                ...
                                <div class="w-full text-left flex flex-col gap-2">
                                    ...
                                    <a href="#" onclick="closeEditModal<?php echo $quality['id']; ?>()" class="transition-all duration-100 ease-in-out h-10 cursor-pointer flex items-center justify-center bg-red-500 text-white rounded-xl font-semibold hover:bg-red-700">Close</a>
                                </div>
                            </form>
                        </div>
                    </div>
                    ...
                    <script>
                        function openEditModal<?php echo $quality['id']; ?>() {
                            const modal = document.getElementById('editModal<?php echo $quality['id']; ?>');
                            const box = document.getElementById('editmodalBox<?php echo $quality['id']; ?>');

                            modal.classList.remove("opacity-0", "pointer-events-none");
                            box.classList.remove("scale-95");
                            box.classList.add("scale-100");
                        }
                        function closeEditModal<?php echo $quality['id']; ?>() {
                            const modal = document.getElementById('editModal<?php echo $quality['id']; ?>');
                            const box = document.getElementById('editmodalBox<?php echo $quality['id']; ?>');
                        ...
                        document.getElementById('editModal<?php echo $quality['id']; ?>').addEventListener("click", function(e) {
                            if (e.target === this) {
                                closeEditModal<?php echo $quality['id']; ?>();
                            }
                        });
                    </script>
