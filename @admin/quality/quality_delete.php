...
<body>
    <?php
        ...
        $section = "quality";
        $slug = trim($_GET['slug']);
        $deletequality = "DELETE FROM types WHERE `slug` = '".$slug."' AND `section` = '".$section."';";

        $result = mysqli_query($conn, $deletequality);
        if (!$result) {
